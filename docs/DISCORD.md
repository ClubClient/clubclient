...
Заведи ещё **`#mod-log`** — приватный канал (доступ только у роли `Club Dev`), в настройках AutoMod
укажи его как канал оповещений. Тогда ты видишь, что он заблокировал, и не гадаешь.
...
7. **AutoMod** — три правила + список ключевых слов, и приватный `#mod-log` (§4a). Ботов не ставить.
8. Настройки сервера → **Эмодзи** → загрузить 15 PNG из `build/discord/emoji/`.
9. Вебхук GitHub → `#changelog` (§5).
10. Запостить и **закрепить** тексты в `rules` и `support` (§6).
11. Баннер — **потом**, когда будут бусты и трафик.
