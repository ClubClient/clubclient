...
- **Карточки** (`ModuleTile`, приватный класс экрана; анатомия Stage 11): чип 28px с MSDF-иконкой +
  **полоса состояния 16×3 строго по центру под чипом** + имя + ghost-подложка (та же иконка 44px,
  5–10% альфы, у правого края под клипом). Состояние = только ЦВЕТ (серый ↔ цвет категории одним
