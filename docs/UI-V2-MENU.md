...
- **Поиск** (`SearchField`, приватный класс экрана): live-фильтр по имени+описанию модуля,
  иконка-лупа из атласа. **Reflow (Stage 21, хореография владельца):** неподходящие карточки
  растворяются на месте с 0мс (fade + scale 1→0.97, ~130мс) и покидают layout только после
  анимации (слой `leaving` под клипом скролла); выжившие скользят к новым слотам с +40мс;
  новые появляются fade+scale 0.97→1 с +70мс. Контейнер не двигается. Быстрая печать НЕ
  перезапускает анимацию: моушн в `tileMotion` (ключ — Module) переживает ребилды, каждый
  символ лишь ретаргетит те же переходы (fade пересеивается от текущего значения при смене
  направления). Стаггер (17мс/карточка) — ТОЛЬКО при открытии/смене категории, в поиске всё
  одновременно.
