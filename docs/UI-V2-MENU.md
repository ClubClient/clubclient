...
- **Окно фиксировано по центру** 860×380 (владелец, 2026-07-02: перетаскивание и грип УДАЛЕНЫ;
  `ClubConfig.menuX/menuY` удалены — старые ключи в json игнорируются). Сетка — **ровно 4 колонки**
  (разреженные ряды — нормально). Открытие: подъём окна 12px; закрытие: та же анимация в обратную
  сторону, реальный `setScreen(null)` — по её завершении.
